...
test.describe('Low Rate Discount - Rule ID Verification', () => {
  ...
  test('Should check which discount rule is being applied (April 5, 2026)', async ({ page }) => {
    ...
    await page.screenshot({ 
      path: `screenshots/low-rate-rule-verification.png`,
      fullPage: true 
    });
    ...
    const fs = require('fs');
    fs.writeFileSync('screenshots/page-content-april-5.html', html);
    console.log('💾 Saved full page HTML to: screenshots/page-content-april-5.html');
  });
  ...
  test('Should check discount rule outside date range (March 31, 2026)', async ({ page }) => {
    ...
    await page.screenshot({ 
      path: `screenshots/low-rate-rule-verification-march-31.png`,
      fullPage: true 
    });
  });
});
