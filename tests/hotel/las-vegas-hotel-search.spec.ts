...
test.describe('Las Vegas Hotel Search - Price Categories', () => {
  ...
  test('Should search Las Vegas hotels for April 5, 2026 and categorize by price', async ({ page }) => {
    ...
    await page.screenshot({ 
      path: 'test-results/hotel/screenshots/las-vegas-search-results.png',
      fullPage: true 
    });
    console.log('📸 Screenshot saved: test-results/hotel/screenshots/las-vegas-search-results.png');
    ...
    fs.writeFileSync('test-results/hotel/screenshots/las-vegas-hotels-by-price.json', JSON.stringify(resultsData, null, 2));
    console.log('💾 Results saved to: test-results/hotel/screenshots/las-vegas-hotels-by-price.json\n');
  });
  ...
  test('Should test Low Rate Discount on hotels under $100', async ({ page }) => {
    ...
    try {
      const data = fs.readFileSync('test-results/hotel/screenshots/las-vegas-hotels-by-price.json', 'utf8');
