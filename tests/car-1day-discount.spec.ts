...
const SCREENSHOT_DIR = 'test-results/car-screenshots/1-day';
...
const TEST_CAR_PARAMS = {
  ...
  dropoff_time: '10:00',
  renter_age: '30',
  // TODO: Add specific car_id, location_id from actual search results
};
...
test.describe('Car Rental 1-Day Discount Tests - March 2026', () => {
  ...
  test.beforeAll(async () => {
    ...
    reportContent.push('**JWT:** Production JWT (same as hotel DOW testing)');
  ...
  test.describe('Period 1: March 1-7, 2026 (12% Fixed Discount)', () => {
    ...
    for (const tc of period1Tests) {
      test(`${tc.pickupDate} (${tc.day}): Expect 12% discount`, async ({ page }) => {
        ...
        reportContent.push('');
        reportContent.push(`![Screenshot](car-screenshots/1-day/${screenshotName}.png)`);
        reportContent.push('');
        reportContent.push('---');
        reportContent.push('');
