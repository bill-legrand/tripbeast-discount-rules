...
test.describe('Las Vegas Hotels - Price Check for Low Rate Discount', () => {
  ...
  test('Should check multiple Las Vegas hotels and categorize by price', async ({ page }) => {
    ...
    // Save results
    const fs = require('fs');
    fs.writeFileSync('screenshots/las-vegas-hotels-price-results.json', JSON.stringify({
    ...
    console.log('💾 Results saved to: screenshots/las-vegas-hotels-price-results.json\n');
  });
});
